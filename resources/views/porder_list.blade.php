  ...
  <div class="row menu-bar">
    <div class="col-md-10">
      <div class="btn-group btn-group-custom">
        <button class="menu_button" onclick="porderView()"><i class="fa fa-plus"></i><br> Create New<br>P. Order</button>
        <button class="menu_button" onclick="viewPorder()"><i class="fa fa-pencil-square-o"></i><br> View/Edit<br>P. Order</button>
        <button class="menu_button" onclick="deletePorders()"><i class="fa fa-times"></i><br> Delete<br>Selected</button>
        ...
        <div class="hide_in_mobile" style="border-left:2px solid grey;height:66px; margin:2px;"></div>
        <button class="menu_button" data-toggle="modal" data-target="#createNewApp"><i class="fa fa-search"></i><br> Search In<br>P. Orders</button>
        ...
        <button class="menu_button" data-toggle="modal" data-target="#createNewApp"><i class="fa fa-refresh"></i><br> Refresh<br>P. Orders List</button>
        <div class="hide_in_mobile" style="border-left:2px solid grey;height:66px; margin:2px;"></div>
      ...
      <div class="row" id="sum">
        <div class="col-md-12" style="font-size: 12px">
          <table class="table" style="text-align: right;">
              <tr>
                <td style="padding: 0px;width:10%;text-align: left">{{$count}} P. Order(s)</td>
...
<script>
  function porderView(){
    $.post("{{route("porder.new")}}",{"_token":"{{csrf_token()}}"},function(id, status){
      if(status=="success"){
  ...
  function viewPorder(){
    window.location = "{{route("porder.new")}}/" + firstID();
  }
  ...
  $(".f").on('change', function(){
    ...
    ap = ap.slice(1);
    if(this.type != 'date'){
      window.location = "{{ route('porder.index') }}?"+ap;
    }
  });

  function tabView(id){
    $.get("{{ route('tabs.view',['ind'=>3]) }}/"+id,function(data,status){
      if(status=='success'){
        $("#tabs_view").html(data);
  ...
  @if($count>0)
    tabView({{$porders[0]->id}});
  @endif

  function deletePorders(){
    if(!confirm('Are you sure you want to delete..?')){
      return;
    }
    var p ={'_token':'{{csrf_token()}}'}
    p['items'] = sarr();
    $.post("{{ route('porder.delete') }}",p,function(data,status){
      if(status=='success'){
  ...
  function printView(){
    window.location = "{{ route('print.view', ['ind'=>3]) }}/"+firstID();
  }
