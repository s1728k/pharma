  ...
  <div class="row">
    ...
    <div class="col-md-8" style="border:1px solid gray">
      <div class="row menu-bar">
        <div class="col-md-12">
          <div class="btn-group btn-group-custom">
            ...
            <div class="hide_in_mobile" style="border-left:2px solid grey;height:66px; margin:2px;"></div>
            <button class="menu_button" onclick="printView()"><i class="fa fa-search-plus"></i><br> Preview<br>Estimate</button>
            <button class="menu_button" data-toggle="modal" data-target="#createNewApp"><i class="fa fa-print"></i><br> Print<br>Estimate</button>
            <div class="hide_in_mobile" style="border-left:2px solid grey;height:66px; margin:2px;"></div>
            ...
            <div class="hide_in_mobile" style="border-left:2px solid grey;height:66px; margin:2px;"></div>
            <button class="menu_button" data-toggle="modal" data-target="#createNewApp"><i class="fa fa-qrcode"></i><br> Mark estimate<br>as 'Paid'</button>
            <div class="hide_in_mobile" style="border-left:2px solid grey;height:66px; margin:2px;"></div>
            <button class="menu_button" data-toggle="modal" data-target="#createNewApp"><i class="fa fa-low-vision"></i><br> Void<br>Estimate</button>
            <div class="hide_in_mobile" style="border-left:2px solid grey;height:66px; margin:2px;"></div>
      ...
      <div class="row">
            ...
            <div class="row">
              <div class="col-md-6">
                <div class="row">
                  ...
                  <div class="col-md-9">
                    <select type="text" style="width: 100%; margin-bottom: 4px" name="estimate_to" class="i a1">
                      <option></option>
                      @foreach($customers as $customer)
                      <option @if($i->estimate_to == $customer->bname) selected @endif value="{{$customer->id}}">{{$customer->bname}}</option>
                      @endforeach
                    </select>
                  </div>
                </div>
        ...
        <div class="col-md-4">
          <div style="border:1px solid grey; padding: 10px; margin-top: 10px;display: inline-block; width: 100%; font-size: 12px; position: relative;">
            <span style="position: absolute;top:-8px;left:10px;background: #eee">Estimate</span>
            <div class="row">
              <div class="col-md-3" style="padding-right: 0px">
                Estimate#
              </div>
              <div class="col-md-9">
                <input type="text" style="width: 100%; margin-bottom: 4px" name="estimate_no" value="{{$i->estimate_no}}" class="i">
              </div>
            </div>
            <div class="row">
              <div class="col-md-3" style="padding-right: 0px">
              ...
              <div class="col-md-9">
                <input type="date" style="width: 100%; margin-bottom: 4px" name="estimate_date" value="{{$i->estimate_date}}" class="i">
              </div>
            </div>
            <div class="row">
              <div class="col-md-3" style="padding-right: 0px">
          ...
          <div style="border:1px solid grey; padding: 10px; margin-top: 10px;margin-bottom: 10px;; width: 100%; font-size: 12px">
            ...
            <div class="tab-content">
              ...
              <div id="documents" class="tab-pane fade">
                <div class="row" style="margin-top: 10px">
                  <div class="col-md-12">
                    <p>Attached documents or image files. If you attach large files to estimate email then emails take long time to send. <button style="float: right">-</button><button style="float: right;">+</button></p>
                    <table class="table table-bordered table-custom">
                      <thead>
                        <tr>
          ...
          <div style="border:1px solid grey; padding: 10px; margin-top: 10px;margin-bottom: 10px;; width: 100%; font-size: 12px">
            ...
            <table class="table table-bordered table-custom">
              <tbody>
                ...
                <tr>
                  <td style="padding: 0px">Estimate total</td>
                  <td style="padding: 0px">{{$setting->currency_sign}} {{number_format($i->estimate_total,2)}}</td>
                </tr>
                <tr>
...
<script>
  function customerDialog(){
    window.location = "{{ route('customer.filter',['ind'=>2,'id'=>$i->id]) }}";
  }

  function productDialog(){
    window.location = "{{ route('product.filter',['ind'=>2,'id'=>$i->id]) }}";
  }
  ...
  $(".i").on('change', function(){
    ...
    $.post("{{ route('estimate.save') }}",p,function(data,status){
      if(status=='success'){
        if(data!=0){
          window.location = "{{ route('estimate.new') }}/"+data;
        }
      }
    });
  });
  ...
  function deleteItems(){
    ...
    $.post("{{ route('item.delete') }}",p,function(items,status){
      if(status == 'success'){
        window.location = "{{ route('estimate.new') }}/{{$i->id}}";
      }
    });
  }
  ...
  function printView(){
    window.location = "{{ route('print.view', ['ind'=>2,'id'=>$i->id]) }}";
  }
</script>
