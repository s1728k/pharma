  ...
  <div class="row menu-bar">
    <div class="col-md-10">
      <div class="btn-group btn-group-custom">
        <button class="menu_button" onclick="orderView()"><i class="fa fa-plus"></i><br> Create New<br>Order</button>
        <button class="menu_button" onclick="viewOrder()"><i class="fa fa-pencil-square-o"></i><br> View/Edit<br>Order</button>
        <button class="menu_button" onclick="deleteOrders()"><i class="fa fa-times"></i><br> Delete<br>Selected</button>
        ...
        <div class="hide_in_mobile" style="border-left:2px solid grey;height:66px; margin:2px;"></div>
        <button class="menu_button" data-toggle="modal" data-target="#createNewApp"><i class="fa fa-search"></i><br> Search In<br>Orders</button>
        ...
        <button class="menu_button" data-toggle="modal" data-target="#createNewApp"><i class="fa fa-refresh"></i><br> Refresh<br>Orders List</button>
        <div class="hide_in_mobile" style="border-left:2px solid grey;height:66px; margin:2px;"></div>
      ...
      <div class="row" id="sum">
        <div class="col-md-12" style="font-size: 12px">
          <table class="table" style="text-align: right;">
              <tr>
                <td style="padding: 0px;width:10%;text-align: left">{{$count}} order(s)</td>
...
<script>
  function orderView(){
    $.post("{{route("order.new")}}",{"_token":"{{csrf_token()}}"},function(id, status){
      if(status=="success"){
  ...
  function viewOrder(){
    window.location = "{{route("order.new")}}/" + firstID();
  }
  ...
  $(".f").on('change', function(){
    ...
    ap = ap.slice(1);
    if(this.type != 'date'){
      window.location = "{{ route('order.index') }}?"+ap;
    }
  });

  function tabView(id){
    $.get("{{ route('tabs.view',['ind'=>1]) }}/"+id,function(data,status){
      if(status=='success'){
        $("#tabs_view").html(data);
  ...
  function deleteOrders(){
    ...
    p['items'] = sarr();
    $.post("{{ route('order.delete') }}",p,function(data,status){
      if(status=='success'){
  ...
  function printView(){
    window.location = "{{ route('print.view', ['ind'=>1]) }}/"+firstID();
  }
  ...
  function convertToInvoice(){
    ...
    $('.v').each(function(i){
      if($('.v').eq(i).prop('checked')){
        if(!id){id=(this.name).replace('v','')}
      }
    })
    var p = {'_token':'{{csrf_token()}}'};
    p['id'] = id;
    $.post('{{ route('order.invoice') }}',p,function(iid,status){
      if(status=='success'){
